...
double compute_loss(double out[3], double C, double J, data X, double **w) {
...
#if FULL_L2
  // compute ||w||^2
  for ( j = 0; j < X.numblocks; j++) {
    for ( k = 0; k < X.blocksizes[j]; k++) {
  ...
  for ( c = 0; c < X.numcomponents; c++) {
    double val = 0;
    for ( i = 0; i < X.componentsizes[c]; i++) {
      int b = X.componentblocks[c][i];
      double blockval = 0;
      for ( k = 0; k < X.blocksizes[b]; k++)
  ...
  for ( l = 0; l <= 1; l++) {
    ...
    for ( i = 0; i < X.num; i++) {
      ...
      int M = -1;
      double V = -10000000000000000000000.0;
      for ( m = 0; m < x.num; m++) {
...
void gd(double C, double J, data X, double **w, double **lb, char *logdir, char *logtag) {
 /* ofstream logfile;
  ...
  while (t < ITER && !converged) {
    ...
    for ( i = 0; i < num; i++)
      perm[i] = i;
    for ( swapi = 0; swapi < num; swapi++) {
          //swapj = (int)(rand()*(num-swapi)) + swapi;  
		swapj = (int)((float)rand()/RAND_MAX*(num-swapi)) + swapi;  
	...
	if (W[i] == INCACHE)
		  ...
		  W[i] = MINWAIT + (int)((float)rand()/RAND_MAX*50);
	else
	  W[i]++;
      }
      ...
      if (t % REGFREQ == 0) {
        ...
        for ( i = 0; i < X.componentsizes[maxc]; i++) {
...
int main(int argc, char **argv) {  
  ...
  for ( i = 0; i < X.numcomponents; i++) {
    ...
    for ( j = 0; j < X.componentsizes[i]; j++)
