...
%使用模型和分数阈值在特性金字塔检测对象。高阈值会导致更少的检测。
...
% dets一共六列，一行代表一个检测器。前四列表示每个检测框坐标，第五列表示每个检测器
% 使用的组件，第六列表示每个检测器的分数
...
for level = model.interval+1:length(pyra.scales)
  ...
  %ind2sub用于把数组中元素索引值转换为该元素在数组中对应的下标
  X = [X; tmpX];  %加在向量后面
  Y = [Y; tmpY];
  I = [I; tmpI];
...
%从高到低排序，只要最高的   descend表示降序   默认是升序
